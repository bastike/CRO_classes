...
class cDistance2Line():
    ...
    def linearEuklidicDistanceSearch(self,croline,vehicletrack):
        
        index = np.argmin(np.linalg.norm(croline-vehicletrack,axis=1))
    ...
    def calcDistanceAzimutVector(self,Line,vehiclepoi,index):
    ...
    def loadCROazimut(self,dfCROLayer1Road,lindex):
        
        roadazimut = dfCROLayer1Road.bearing.values[lindex]
    ...
    def distance2line(self,vector,roadazimut):
        
        if vector[1] >= 355 and roadazimut <= 5:
            d2l = math.sin(math.radians(360-vector[1]+roadazimut)*abs(vector[0]))

        else:
            d2l = math.sin(math.radians(vector[1]-roadazimut))*abs(vector[0])
