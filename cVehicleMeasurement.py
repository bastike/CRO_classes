...
class cVehicleMeasurement():
    ...
    def __init__(self):
        ...
        self.listofLines = ['M0','P0',
                   ...
                   'P3','P4']
        #all excisting roads
        self.roadpolypath = self.sGrandParentDir+'/data/road/AdriveRoadsSHP/ADriveRoads.shp'
        #all ADrive Roads
    ...
    def importVehicledata(self,path,filename):
        """
        import vehicle data and give pandas DataFrame back
        @:param path              [in]   string path to mat file
        @:param filename          [in]   string filename
        @:param df                [out]  vehicle data as pandas DataFrame
        """
        mat = sio.loadmat(self.sGrandParentDir+'//data//vehicle//mat//'+filename)
        dfvehicle = pd.DataFrame(np.hstack((mat['Data1_INS_Lat_Abs_POI4___Latitu'],
                                            mat['Data1_INS_Long_Abs_POI4___Longi'])),columns=['lat','lon'])
        return dfvehicle
    ...
    def checkWhichRoad(self,lon,lat):
